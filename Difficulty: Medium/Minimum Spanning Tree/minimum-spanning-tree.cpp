...
class Solution {
  public:
    ...
    int spanningTree(int V, vector<vector<int>> adj[]) {
        ...
        // we relax only during pop as we are sure then node,wt edge will be included in MST
        // no need for relax during pushing into PQ
        
        // nei node makes the overall scene better, push only if relaxing
        // *Note: Maintain vis, update after pop, dont update before push just check;
        while(!pq.empty()) { // in pq : u update only when pop, not when push
            ...
            for(auto nei: adj[a]) {
                ...
                // optimise during push, but update at pop only
                if(wb < dist[b]) {
                    pq.push({wb,b});
                } 
            }
